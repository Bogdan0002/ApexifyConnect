...
class ApplicationServiceTest {
    ...
    void setUp() {
        ...
        applicationService = new ApplicationService(applicationRepository, jobPostDAO, userDAO);
    ...
    void getApplicationsForJob_Success() {
        ...
        when(applicationRepository.findByJobPostId(1L)).thenReturn(applications);

        List<Application> result = applicationService.getApplicationsForJob(1L);

        assertEquals(1, result.size());
        verify(applicationRepository).findByJobPostId(1L);
    }

    @Test
    void getApplicationsForJob_NoApplications() {
        when(applicationRepository.findByJobPostId(1L)).thenReturn(Collections.emptyList());
